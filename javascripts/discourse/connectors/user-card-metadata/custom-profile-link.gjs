...
export default class CustomProfileLink extends Component {
  get links() {
    ...
    if (!this.args.outletArgs.user.get("user_fields")) {
      ...
      for (let i = 0; i < ids.length; i++) {
        if (
          !this.args.outletArgs.user.get("user_fields")[ids[i]] &&
          settings.custom_profile_link_debug_mode
        ) {
          console.debug(
            `[Custom Profile Link] User field ${ids[i]} missing. user_fields dump follows.`,
            this.args.outletArgs.user.get("user_fields"),
          );
        }
        if (this.args.outletArgs.user.get("user_fields")[ids[i]]) {
          links.push({
            value: this.args.outletArgs.user.get("user_fields")[ids[i]],
                     label: labels[i],
                     prefix: prefixes[i],
          });
        }
  ...
  get orgs() {
    ...
    if (!this.args.outletArgs.user.get("user_fields")) {
      ...
      for (let i = 0; i < orgsData.length; i++) {
        ...
        if (
          (!this.args.outletArgs.user.get("user_fields")[data[1]] ||
          !this.args.outletArgs.user.get("user_fields")[data[2]]) &&
          settings.custom_profile_link_debug_mode
        ) {
          console.debug(
        ...
        if (
          ...
          orgs.push({
            label: data[0],
            name: this.args.outletArgs.user.get("user_fields")[data[1]],
                    url: this.args.outletArgs.user.get("user_fields")[data[2]],
          });
        }
      }
    }
  ...
  <span class="profile-link"><a href="{{link.prefix}}{{link.value}}" target="_blank">{{link.label}}</a></span>
  {{/each}}
  </div>
  {{/if}}
  {{#if this.orgs}}
  <div class="user-card-metadata-outlet custom-profile-links-orgs">
  {{#each this.orgs as |org|}}
  <span class="desc">{{org.label}}</span>
  <span><a href="{{org.url}}" target="_blank">{{org.name}}</a></span>
  {{/each}}
  </div>
  {{/if}}
  </template>
